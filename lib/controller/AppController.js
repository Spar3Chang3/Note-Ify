...
export default class AppController {
    ...
    if (session) {
      ...
      if (sessionPlaying) {
        ...
        if (closed) {
          ...
          console.log(this.sessionCloser);
        }
      }
    }
  ...
  async handleStop(message) {
    ...
    setTimeout((id = s.getId()) => {
      const guildClosed = this.sessionCloser.get(message.guild.id);
      if (guildClosed) {
        guildClosed.delete(id);
        if (guildClosed.size === 0) {
          this.sessionCloser.delete(message.guild.id);
        }
      }
  ...
  async handlePause(message) {
    ...
    await s.pause();
    await message.reply(
      "I've left the call while you guys take a break. Just send `@bot unpause` whenever you're ready to resume playing!",
    );
  }
  ...
  async handleUnpause(message) {
    ...
    await s.unpause();
    await message.reply(
      `Rejoined ${message.member.voice.channel} and listening.`,
    );
  }
}
