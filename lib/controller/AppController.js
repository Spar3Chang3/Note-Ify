...
export default class AppController {
  ...
  async handleMessage(message) {
    try {
      ...
      if (ExtractUserId(fullCommand[0]) !== client.user.id) {
        message.reply(
  ...
  async handleStart(message, args) {
    ...
    if (session) {
      ...
      if (sessionPlaying) {
        ...
        if (!sessionPlaying.hasTrustee(message.user.id)) {
          await message.reply(
    ...
    const targetGm = ExtractUserId(
      flagArgs[COMMAND_LIST.start.flags.gm] || "",
    )[0];
